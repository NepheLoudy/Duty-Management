...
function getPolicy() {
  ...
  return {
    ...
    p2pCommands: [
      '值日助手', '我要请假', '确认请假', '取消请假', '查询我的下一次值日', '是', '否', '打卡', '打卡了', '生成排班表',
      '是的', '好', '好了', '完成', '完成了', '做完了', '搞定', '搞定了',
      '快递助手', '快递', '查询当前快递', '已取', '全部已取', '确认全部已取',
      '/值日助手', '/我要请假', '/确认请假', '/取消请假', '/查询我的下一次值日', '/是', '/否', '/打卡', '/打卡了', '/生成排班表',
      '/快递助手', '/快递', '/查询当前快递',
    ],
    p2pCommandPrefixes: ['绑定', '/绑定'],
    // 取件回复词形（已取n / 全部已取 / 确认全部已取）：p2p 与群 @ 转发放行依据（正则，hub 消费）；
    // 「确认全部已取」为「全部已取」两步确认的复核词（2026-09-27 新增）
    p2pCommandPatterns: ['^已取\\s*\\d*$', '^全部已取$', '^确认全部已取$'],
