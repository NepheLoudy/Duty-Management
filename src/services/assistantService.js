...
const HELP_TEXT = [
  '🧹 值日助手用法（指令带不带 / 都可以）：',
  '· 「查询我的下一次值日」—— 下次值日日期与岗位',
  '· 「我要请假」—— 请假分两步：先发「我要请假」，机器人点名班次日期，回复「确认请假」才生效（10 分钟内有效，误触回「取消请假」）；生效后当次置已请假（当日该岗空缺，由同日队员兼顾），下周自动补插一次值日',
  '· 「绑定 姓名」—— 首次使用绑定账号（提醒与确认走私信）',
  '· 「打卡」—— 值日日 18:30 询问后回复它完成打卡（口语「是的」「完成了」也可；照片可直接发我，24:00（午夜）收口）；「否」= 今天不做了',
  '· 「生成排班表」—— 管理员专用，按日历向下生成一个月排班',
  '',
  '规则：完成后回复「打卡」并上传现场照片（照片写入值日表对应岗位栏）；24:00（午夜）收口，未打卡记为未做完。',
...
function buildBoardCard(dateStr, records, yesterday) {
  ...
  elements.push(
    { tag: 'hr' },
    { tag: 'markdown', content: '> 排班查询 / 请假 / 打卡确认均在私信办理：私信机器人「值日助手」可查询排班、请假；完成后请私信回复「打卡」并上传照片，24:00（午夜）收口。' },
  );
