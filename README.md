...
- **请假当日不补位**（其余成员照常值日，当日总状态因缺凭证记未完成）。
...
| 每日 12:00 | 看板自动播报 | 今日值日看板卡片经群自定义机器人 webhook 推到值日播报群；未配置 webhook 或推送失败时回退应用身份直发管辖群（两者都不可用才跳过）；无排班记录跳过 |
...
- 私信（精确匹配）：`值日助手`（用法说明）、`查询我的下一次值日`、`我要请假`、
  `绑定 姓名`、`是` / `否`、`生成排班表`（仅管理员）；打卡确认口语变体
  （`是的`/`好`/`完成了`/`做完了`/`搞定` 等）在**有当日活跃询问会话**时等同「是」，
  无会话静默交还 hub（不发未识别提示，不接管闲聊）；
...
5. pm-robot `.env`：`DUTY_CHAT_ID` / `DUTY_WEBHOOK_URL`（M4 播报上线时生效）。
