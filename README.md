...
- **缺勤补偿**（可突破轮次上限）：已请假/未做完 → 下周（自然周）插入 +1 次：
  ...
  已生成但该队员**当周天天有班**（小队多条补偿同周）则自动顺延下一周（deferCount 计数、对账报告可见，
  2026-09-19 起，不再静默留队到过期）；**连续两次「未做完」**额外多插 1 次（该缺勤周期共 3 次；
  2026-09-24 收紧：**主动请假不计入**连续缺勤——合规安排不同罪，防请假→加罚→班更多→更易被抽调的雪球）；
- **请假当日补位**（2026-09-12 口径：请假必须有补位）：登记请假时立即从队列抽调一人
  当日顶上——候选为值日队列中在其后仍有排班者，同岗优先、**近 14 天已值次数最少者优先**
  （2026-09-24 改口径：旧版按「远期排班日最远」当余量，会把被补偿/加罚插入多班次的人反复选中）；
  抽调是**加插**而非对调（被抽调者远期班次保留，私信告知补位），请假人仍进下周补偿（总量守恒）。
  找不到可抽调人选时当日空缺、管理员回执可见；未做完（收口才发现）无法当日补位，仍走下周补偿；
- **请假两步确认**（2026-09-24）：「我要请假」只登记意向并点名班次日期，回复「确认请假」才生效
  （10 分钟内有效，误触回「取消请假」）——旧版直取最近班次立即置请假，误触一次就少一个班且牵动补位/补偿。
